...
本目录自包含新程序、新结果和新图，原有 `代码/q2_solution.py`、`代码/results/q2`、`代码/figures/q2`、原始附件和论文不参与写入。模型细节见 [建模思路/问题二/概率净负荷模型合同](../../../建模思路/问题二/第二问_概率净负荷模型合同.md)，结果阅读版见 [论文写作/问题二/独立实验结果报告](../../../论文写作/问题二/第二问_独立概率净负荷结果报告.md)。
...
`--stage smoke` 是真实输入最小纵向测试；`forecast` 仅生成全年预测；`run` 使用预测缓存运行控制回测；`export` 重新核验及导出现有完整结果。默认 `all` 按代码、输入和依赖签名检查逐日缓存，支持断点继续。需要完全从零运行时，复制本目录的两个 `.py`、`.ps1`、[模型合同](../../../建模思路/问题二/第二问_概率净负荷模型合同.md)和原文件哈希清单到同层的另一个新实验目录，不复制 `results`、`figures`，再运行；根目录按固定相对层级解析。
...
- `../../../论文写作/问题二/第二问_独立概率净负荷结果报告.md`：由 `make_report.py` 从实际输出生成的阅读版报告。
