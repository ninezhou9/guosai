...
本参赛队在竞赛过程中使用了 ChatGPT，主要用于代码检查与调试、基于 MATLAB 的科研绘图与结果可视化，以及时间标签核对、跨日补算实现、结果模板回填和支撑材料整理；参赛队已人工讨论并修改建模方案，并核验代码。详见支撑文件。

# 附录

## A 支撑文件列表

1. AI使用说明.docx：AI工具的使用范围与方式。

2. 源码/代码/：四问模型、实验与绘图程序，以及结果导出和跨日补算程序。主版本与运行顺序见使用说明.md。

3. 结果/：result1.xlsx、result2.xlsx、result3.xlsx、result4-2.xlsx、result4-3.xlsx；问题一至问题四子目录保留原模型的汇总和明细。

4. 结果/跨日补算/：跨日预测记录、模板回填核验记录和时间口径说明。

5. 使用说明.md：目录说明、复现顺序及结果时间口径。

## B 核心源码

以下列出主要计算函数。代码依赖、参数和调用入口见对应完整源码；原模型与新增跨日处理分开保存。

### 问题一

源码位置：源码/代码/q1_solution.py
...
源码位置：源码/代码/实验/q2_fresh_20260912/fresh_engine.py
...
源码位置：源码/代码/实验/q3_revision_a_20260912/q3_engine.py
...
源码位置：源码/代码/实验/q4_independent_codex_20260912/model.py
...
源码位置：源码/代码/结果导出/boundary_rules.py
...
完整跨日计算见源码/代码/结果导出/boundary_prediction.py；模板数据准备、回填和验证分别见prepare_templates.py、refill_templates.mjs与verify_templates.py。
