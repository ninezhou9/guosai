...
def sync_files():
    ...
    old_note=ROOT/'论文写作/历史版本/原模板重填_20260913/请先阅读_重填状态.txt'
    if old_note.exists() and not (records/'时间口径说明.txt').exists():write_target(old_note,records/'时间口径说明.txt')
    for name in NAMES:
    ...
    for folder,dirs,files in os.walk(ROOT/'代码',followlinks=False):
        ...
        for name in files:
            ...
            if source.suffix.lower() not in ['.py','.m','.ps1','.mjs','.js'] and source.name not in ['README.md','requirements.txt']:continue
