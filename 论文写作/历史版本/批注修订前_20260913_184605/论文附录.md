...
2. 代码/：按问题一至四、时间对齐和绘图分组，收录最终模型及其核验程序。具体入口与运行顺序见使用说明.md。
...
以下列出主要计算函数。代码依赖、参数和调用入口见对应完整源码；支撑包仅适配目录路径，原模型与新增跨日处理分开保存。

### 问题一

源码位置：代码/问题一/q1_solution.py
...
源码位置：代码/时间对齐/boundary_rules.py
...
完整跨日计算见代码/时间对齐/boundary_prediction.py；同目录的prepare_templates.py、refill_templates.mjs与verify_templates.py分别准备、回填和验证模板数据。
