...
from boundary_rules import solve_next_cycle, extend_pv_hours
...
def q1():
    import csv
    import numpy as np
    original = ROOT/'代码/问题一/q1_solution.py'
    before = digest(original)
    m = module('boundary_q1_original', original)
    p, load, pv = m.load()
    with (ROOT/'代码/问题一/results/完整调度.csv').open(encoding='utf-8-sig', newline='') as f:
    ...
    fit, (g, c, discharge, spill), energy = solve_next_cycle(m.solve, p, load, pv, previous_end)
    checks = dict(
...
def q3():
    ...
    next_hourly = extend_pv_hours(forecasts)
    estimated = next_hourly[18:]
