...
def report():
    a=json.loads((OUT/'annual.json').read_text(encoding='utf-8'))
    ...
    lines+=['',f'4-2 主结果为 {fixed["total_cost"]:,.2f} 元；4-3 主结果为 {main["total_cost"]:,.2f} 元。',
        ...
        '复现顺序：python -X utf8 run.py --jobs 9 → python -X utf8 deliver.py --sensitivity。',
...
def manifest():
    outputs={str(p.relative_to(m.BASE)):m.digest(p) for folder in [OUT,m.BASE/'figures'] for p in folder.glob('*') if p.is_file() and p.name not in ['manifest.json','复现清单.json']}
    ...
    data=dict(inputs=m.provenance(),sources={p.name:m.digest(p) for p in m.BASE.glob('*.py')},outputs=outputs,
              ...
              reproduction='python -X utf8 run.py --jobs 9; python -X utf8 deliver.py --sensitivity')
